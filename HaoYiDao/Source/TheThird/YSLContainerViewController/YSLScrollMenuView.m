...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _itemTitleColor = [UIColor colorWithRed:0.866667
                                          green:0.866667
                                           blue:0.866667
                                          alpha:1.0];
        _itemSelectedTitleColor = [UIColor colorWithRed:0.333333
                                                  green:0.333333
                                                   blue:0.333333
                                                  alpha:1.0];
        _itemIndicatorColor = [UIColor colorWithRed:0.168627
                                              green:0.498039
                                               blue:0.839216
                                              alpha:1.0];
        
#pragma mark 添加编辑的button
        ...
        self.editBt = [UIButton buttonWithType:(UIButtonTypeRoundedRect)];
        _editBt.frame = CGRectMake(self.frame.size.width - height * .8, height * .2,
                                   height * .6, height * .6);
        [_editBt setImage:[UIImage imageNamed:@"nsews_item_add"]
    ...
    if (_itemTitleArray != itemTitleArray) {
        ...
        for (int i = 0; i < itemTitleArray.count; i++) {
            ...
            [[UITapGestureRecognizer alloc]initWithTarget:self
                                                   action:@selector(itemViewTapAction:)];
        ...
        _indicatorView.frame = CGRectMake(10, _scrollView.frame.size.height - kYSLIndicatorHeight,
                                          kYSLScrollMenuViewWidth, kYSLIndicatorHeight);
...
- (void)setIndicatorViewFrameWithRatio:(CGFloat)ratio isNextItem:(BOOL)isNextItem toIndex:(NSInteger)toIndex
{
    ...
    if (isNextItem) {
        indicatorX = ((kYSLScrollMenuViewMargin + kYSLScrollMenuViewWidth) * ratio ) +
        (toIndex * kYSLScrollMenuViewWidth) + ((toIndex + 1) * kYSLScrollMenuViewMargin);
    } else {
        indicatorX =  ((kYSLScrollMenuViewMargin + kYSLScrollMenuViewWidth) *
                       (1 - ratio) ) + (toIndex * kYSLScrollMenuViewWidth) +
        ((toIndex + 1) * kYSLScrollMenuViewMargin);
    }
    
    if (indicatorX < kYSLScrollMenuViewMargin ||
        indicatorX > self.scrollView.contentSize.width -
        (kYSLScrollMenuViewMargin + kYSLScrollMenuViewWidth)) {
        
        return;
    }
    _indicatorView.frame = CGRectMake(indicatorX,
                                      _scrollView.frame.size.height - kYSLIndicatorHeight,
                                      kYSLScrollMenuViewWidth, kYSLIndicatorHeight);
...
- (void)setShadowView
{
    ...
    view.frame = CGRectMake(0, self.frame.size.height - 0.5,
...
#pragma mark 添加编辑的button
    ...
    if (self.frame.size.width > x) {
        ...
        frame.origin.x = 0;
        frame.size.width = self.frame.size.width - height;
    }
     */
...
{
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(scrollMenuViewSelectedIndex:)])
    {
